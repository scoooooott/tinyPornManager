...
public class BaseToolTipUI extends MetalToolTipUI {
  ...
  public void paint(Graphics g, JComponent c) {
    ...
    if (!mediumWeight && fancyLayout && shadowSize > 0) {
      ...
      big.setClip(0, 0, w, h);
      /*
       * Paint savedPaint = big.getPaint(); Color cHi; Color cLo; if (ColorHelper.getGrayValue(backColor) < 128) { cHi =
       * ColorHelper.brighter(backColor, 10); cLo = ColorHelper.darker(backColor, 20); } else { cHi = ColorHelper.brighter(backColor, 40); cLo =
       * ColorHelper.darker(backColor, 5); } big.setPaint(new GradientPaint(0, borderSize, cHi, 0, h - (2 * borderSize) - shadowSize, cLo));
       */
