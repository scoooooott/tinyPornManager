...
public class MovieChooser extends JDialog implements ActionListener {
	...
	public MovieChooser(Movie movie) {
		...
		contentPanel
				.setLayout(new FormLayout(new ColumnSpec[] { ColumnSpec
						.decode("max(500px;default):grow"), }, new RowSpec[] {
						FormFactory.RELATED_GAP_ROWSPEC,
						FormFactory.DEFAULT_ROWSPEC,
						FormFactory.UNRELATED_GAP_ROWSPEC,
						RowSpec.decode("fill:403px:grow"),
						FormFactory.RELATED_GAP_ROWSPEC,
						FormFactory.DEFAULT_ROWSPEC, }));
		{
			JPanel panelSearchField = new JPanel();
			contentPanel.add(panelSearchField, "1, 2, fill, fill");
			panelSearchField.setLayout(new FormLayout(new ColumnSpec[] {
					...
					new RowSpec[] { FormFactory.DEFAULT_ROWSPEC, }));
			{
				...
				panelSearchField.add(textFieldSearchString,
						"2, 1, fill, default");
		...
		}
		{
					...
					rowSM.addListSelectionListener(new ListSelectionListener() {
						public void valueChanged(ListSelectionEvent e) {
							...
							if (!lsm.isSelectionEmpty()) {
								...
								try {
									MovieChooserModel model = moviesFound
											.get(selectedRow);
									if (!model.isScraped()) {
			...
			}
			{
				...
				panelSearchDetail.setLayout(new FormLayout(new ColumnSpec[] {
						ColumnSpec.decode("left:max(150px;default)"),
						FormFactory.RELATED_GAP_COLSPEC,
						ColumnSpec.decode("max(300px;default):grow"), },
						new RowSpec[] { RowSpec.decode("30px"),
								...
								RowSpec.decode("default:grow"), }));
				{
					...
					panelSearchDetail
							.add(lblMovieName, "1, 1, 3, 1, fill, top");
				}
				{
				...
				}
				{
					...
					panelSearchDetail.add(tpMovieDescription,
							"1, 6, 3, 1, fill, fill");
				}
			}
		}
			...
			contentPanel.add(buttonPane, "1, 6");
			{
				...
				buttonPane
						.setLayout(new FormLayout(new ColumnSpec[] {
								...
								FormFactory.LABEL_COMPONENT_GAP_COLSPEC,
								ColumnSpec.decode("81px"), }, new RowSpec[] {
								FormFactory.LINE_GAP_ROWSPEC,
								RowSpec.decode("25px"), }));
				{
	...
	private class SearchTask extends SwingWorker<Void, Void> {
		...
		public Void doInBackground() {
			...
			try {
				List<MediaSearchResult> searchResult = tmdb
						.search(new SearchQuery(MediaType.MOVIE,
								SearchQuery.Field.QUERY, searchTerm));
				moviesFound.clear();
	...
	protected void initDataBindings() {
		JTableBinding<MovieChooserModel, List<MovieChooserModel>, JTable> jTableBinding = SwingBindings
				.createJTableBinding(UpdateStrategy.READ, moviesFound, table);
		//
		BeanProperty<MovieChooserModel, String> movieChooserModelBeanProperty = BeanProperty
				.create("combinedName");
		jTableBinding.addColumnBinding(movieChooserModelBeanProperty)
		...
		BeanProperty<JTextPane, String> jTextPaneBeanProperty = BeanProperty
				.create("text");
		AutoBinding<JTable, String, JTextPane, String> autoBinding_1 = Bindings
				.createAutoBinding(UpdateStrategy.READ, table,
						jTableBeanProperty_1, tpMovieDescription,
						jTextPaneBeanProperty);
		autoBinding_1.bind();
		//
		BeanProperty<JTable, String> jTableBeanProperty_2 = BeanProperty
				.create("selectedElement.posterUrl");
