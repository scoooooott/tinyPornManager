...
/**
 * An {@link DefaultEventSelectionModel} is a class that performs two simulaneous services. It is a {@link ListSelectionModel} to provide selection
 * tracking for a {@link JTable}. It is also a {@link EventList} that contains the table's selection.
 * 
 * <p>
 * As elements are selected or deselected, the {@link EventList} aspect of this {@link DefaultEventSelectionModel} changes. Changes to that
 * {@link List} will change the source {@link EventList}. To modify only the selection, use the {@link ListSelectionModel}'s methods.
 * 
 * <p>
 * Alongside <code>MULTIPLE_INTERVAL_SELECTION</code>, this {@link ListSelectionModel} supports an additional selection mode.
 * <code>MULTIPLE_INTERVAL_SELECTION_DEFENSIVE</code> is a new selection mode. It is identical to <code>MULTIPLE_INTERVAL_SELECTION</code> in every
 * way but one. When a row is inserted immediately before a selected row in the <code>MULTIPLE_INTERVAL_SELECTION</code> mode, it becomes selected.
 * But in the <code>MULTIPLE_INTERVAL_SELECTION_DEFENSIVE</code> mode, it does not become selected. To set this mode, use
 * {@link #setSelectionMode(int) setSelectionMode(ListSelection.MULTIPLE_INTERVAL_SELECTION_DEFENSIVE)}.
 * 
 * <p>
 * {@link DefaultEventSelectionModel} is <strong>not thread-safe</strong>. Unless otherwise noted, all methods are only safe to be called from the
 * event dispatch thread. To do this programmatically, use {@link SwingUtilities#invokeAndWait(Runnable)} and wrap the source list (or some part of
 * the source list's pipeline) using GlazedListsSwing#swingThreadProxyList(EventList).
 * </p>
 * 
 ...
 */
public final class DefaultEventSelectionModel<E> extends DefaultListSelectionModel implements AdvancedListSelectionModel<E> {
   ...
   * 
   * The {@link DefaultEventSelectionModel} listens to this {@link EventList} in order to adjust selection when the {@link EventList} is modified. For
   * example, when an element is added to the {@link EventList}, this may offset the selection of the following elements.
   * 
   * @param source
   *          the {@link EventList} whose selection will be managed. This should be the same {@link EventList} passed to the constructor of your
  ...
  /**
   * Creates a new selection model that also presents a list of the selection. The {@link DefaultEventSelectionModel} listens to this
   * {@link EventList} in order to adjust selection when the {@link EventList} is modified. For example, when an element is added to the
   * {@link EventList}, this may offset the selection of the following elements.
   * 
   * @param source
   *          the {@link EventList} whose selection will be managed. This should be the same {@link EventList} passed to the constructor of your
   *          {@link DefaultEventTableModel} or {@link DefaultEventListModel}.
   * @param diposeSource
   *          <code>true</code> if the source list should be disposed when disposing this model, <code>false</code> otherwise
   */
  ...
  /**
   * Listens to selection changes on the {@link ListSelection} and fires {@link ListSelectionEvent}s to registered listeners.
   */
   ...
   * 
   * <p>
   * This notifies all listeners with the same immutable ListSelectionEvent.
   */
   ...
   * 
   * <p>
   * First this calculates the smallest range where changes occur. This includes the union of the selection range before and the selection range
   * specified. It then walks through the change and sets each index as selected or not based on whether the index is in the new range. Finally it
   * fires events to both the listening lists and selection listeners about what changes happened.
   * 
   * <p>
  ...
  /**
   * Returns true if the specified index is selected. If the specified index has not been seen before, this will return false. This is in the case
   * where the table painting and selection have fallen out of sync. Usually in this case there is an update event waiting in the event queue that
   * notifies this model of the change in table size.
   */
   ...
   * 
   * Note that the change events fired by this class may include rows that have been removed from the table. For this reason it is advised not to
   * <code>for()</code> through the changed range without also verifying that each row is still in the table.
   */
