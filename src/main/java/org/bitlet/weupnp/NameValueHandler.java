...
public class NameValueHandler extends DefaultHandler {
   ...
   * 
   * Caches the element as {@link #currentElement}, so that it will be stored as a map key when the corresponding value will be read.
   * 
   * @param uri
   ...
   * 
   * It is used to reset currentElement when the XML node is closed. Note: this works only when the data we are interested in does not contain child
   * nodes.
   * 
   * Based on a patch provided by christophercyll and attached to issue #4: http://code.google.com/p/weupnp/issues/detail?id=4
   * 
   * @param uri
   *          The Namespace URI, or the empty string if the element has no Namespace URI or if Namespace processing is not being performed.
   * @param localName
