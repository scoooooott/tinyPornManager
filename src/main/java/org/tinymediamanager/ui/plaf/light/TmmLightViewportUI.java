...
public class TmmLightViewportUI extends BasicViewportUI {
  // Shared UI object
  private static ViewportUI viewportUI;

  public static ComponentUI createUI(JComponent c) {
    if (viewportUI == null) {
      viewportUI = new TmmLightViewportUI();
    }
    return viewportUI;
  }
  ...
  private void paintVerticalGridLines(Graphics g, JComponent c) {
    ...
    if (viewport.getView() instanceof JTable && !(viewport.getView() instanceof TmmTable)) {
    ...
    if (fTable != null) {
      ...
      if (viewport.getParent() instanceof RoundScrollPane) {
        RoundScrollPane scrollPane = (RoundScrollPane) viewport.getParent();
      ...
      for (int i = 0; i < fTable.getColumnCount() - drawColumnCountOffset; i++) {
        ...
        if (x >= 0) {
          g.setColor(AbstractLookAndFeel.getGridColor());
