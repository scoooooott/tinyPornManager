...
public class TmdbMetadataProvider implements IMovieMetadataProvider, IMediaArtworkProvider, IMovieTrailerProvider {
  ...
  private static MediaProviderInfo      providerInfo      = new MediaProviderInfo(MediaMetadata.TMDBID, "themoviedb.org",
  ...
  private static synchronized void initAPI() throws Exception {
    ...
    if (tmdb == null) {
      try {
        tmdb = new TheMovieDbApi(ApiKey.decryptApikey("8XAdwmcn1zEWLdbc30Kco2ZvOIKyxNxGeiL5kpQlEbXMHwhWBCWKzbNZQ/LINTKb"), new HttpClientWrapper(TmmHttpClient.getHttpClient()));
    ...
    try {
      synchronized (tmdb) {
        ...
        for (Trailer tmdbTrailer : tmdbTrailers) {
          ...
          trailer.setProvider(tmdbTrailer.getSite());
          trailer.setUrl(tmdbTrailer.getKey());

          // youtube support
          if ("youtube".equalsIgnoreCase(tmdbTrailer.getSite())) {
            // build url for youtube trailer
            StringBuilder sb = new StringBuilder();
            sb.append("http://www.youtube.com/watch?v=");
            sb.append(tmdbTrailer.getKey());
            if ("hd".equalsIgnoreCase(tmdbTrailer.getSize()) && !tmdbTrailer.getKey().contains("&hd=1")) {
              sb.append("&hd=1");
            }
  ...
  private void prepareDefaultPoster(MediaArtwork ma, MediaScrapeOptions options) {
    for (ImageSizeAndUrl image : ma.getImageSizes()) {
      ...
      if (image.getWidth() >= 1000) {
        if (options.getPosterSize().getOrder() >= PosterSizes.LARGE.getOrder()) {
      ...
      if (image.getWidth() >= 500) {
        if (options.getPosterSize().getOrder() >= PosterSizes.BIG.getOrder()) {
          ma.setDefaultUrl(image.getUrl());
      ...
      if (image.getWidth() >= 342) {
        if (options.getPosterSize().getOrder() >= PosterSizes.MEDIUM.getOrder()) {
          ma.setDefaultUrl(image.getUrl());
      ...
      if (image.getWidth() >= 185) {
        if (options.getPosterSize() == PosterSizes.SMALL) {
          ma.setDefaultUrl(image.getUrl());
  ...
  private void prepareDefaultFanart(MediaArtwork ma, MediaScrapeOptions options) {
    for (ImageSizeAndUrl image : ma.getImageSizes()) {
      ...
      if (image.getWidth() >= 1920) {
        if (options.getFanartSize().getOrder() >= FanartSizes.LARGE.getOrder()) {
          ma.setDefaultUrl(image.getUrl());
      ...
      if (image.getWidth() >= 1280) {
        if (options.getFanartSize().getOrder() >= FanartSizes.MEDIUM.getOrder()) {
          ma.setDefaultUrl(image.getUrl());
      ...
      if (image.getWidth() >= 300) {
        if (options.getFanartSize().getOrder() >= FanartSizes.SMALL.getOrder()) {
          ma.setDefaultUrl(image.getUrl());
