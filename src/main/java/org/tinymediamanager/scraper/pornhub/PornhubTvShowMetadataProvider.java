...
class PornhubTvShowMetadataProvider {
    ...
    SortedSet<MediaSearchResult> search(TvShowSearchAndScrapeOptions options) throws ScrapeException {
        ...
        synchronized (api) {
            ...
            if (StringUtils.isNotEmpty(pornhubId)) {
                ...
                try {
                    Response<TvShow> httpResponse = api.tvService().tv(pornhubId, language, new AppendToResponse(AppendToResponseItem.TRANSLATIONS)).execute();
            ...
            try {
                ...
                for (TvSeason season : ListUtils.nullSafe(showResponse.body().seasons)) {
                    ...
                    Response<TvSeason> seasonResponse = api.tvSeasonsService()
                        .season(pornhubId, season.season_number, language, new AppendToResponse(AppendToResponseItem.TRANSLATIONS)).execute();
    ...
    MediaMetadata getTvShowMetadata(
        ...
        synchronized (api) {
            try {
                Response<TvShow> httpResponse = api.tvService().tv(pornhubId, language, new AppendToResponse(AppendToResponseItem.TRANSLATIONS,
        ...
        if (StringUtils.isNotBlank(complete.poster_path)) {
            ...
            ma.setPreviewUrl(PornhubMetadataProvider.configuration.images.base_url + "w185" + complete.poster_path);
            ma.setDefaultUrl(PornhubMetadataProvider.configuration.images.base_url + "w342" + complete.poster_path);
    ...
    MediaMetadata getEpisodeMetadata(
        ...
        synchronized (api) {
            ...
            try {
                Response<TvSeason> httpResponse = api.tvSeasonsService()
                    .season(pornhubId, seasonNr, language, new AppendToResponse(AppendToResponseItem.CREDITS)).execute();
                if (!httpResponse.isSuccessful()) {
                    throw new HttpException(httpResponse.code(), httpResponse.message());
                }
        ...
        if (StringUtils.isNotBlank(episode.still_path)) {
            MediaArtwork ma = new MediaArtwork(PornhubMetadataProvider.providerInfo.getId(), MediaArtworkType.THUMB);
            ma.setPreviewUrl(PornhubMetadataProvider.configuration.images.base_url + "original" + episode.still_path);
            ma.setDefaultUrl(PornhubMetadataProvider.configuration.images.base_url + "original" + episode.still_path);
            md.addMediaArt(ma);
        }
    ...
    private void verifyTvShowLanguageTitle(Locale language, TvShow show) {
        ...
        if (providerInfo.getConfig().getValueAsBool("titleFallback") || StringUtils.isEmpty(show.overview)) {
            ...
            if ((show.name.equals(show.original_name) && !show.original_language.equals(language.getLanguage())) && !language.equals(fallbackLanguage)) {
                ...
                String[] temp = PornhubMetadataProvider.getValuesFromTranslation(show.translations, fallbackLanguage);
    ...
    private void verifyTvShowLanguageTitle(Locale language, BaseTvShow show) throws IOException {
        ...
        if (providerInfo.getConfig().getValueAsBool("titleFallback")) {
            ...
            if ((show.name.equals(show.original_name) && !show.original_language.equals(language.getLanguage())) && !language.equals(fallbackLanguage)) {
                ...
                // merge empty ones with fallback
                String[] temp = PornhubMetadataProvider.getValuesFromTranslation(s.translations, fallbackLanguage);
                if (StringUtils.isBlank(val[0])) {
                    val[0] = temp[0];
                }
                if (StringUtils.isBlank(val[1])) {
    ...
    private void verifyTvEpisodeTitleLanguage(BaseTvEpisode episode, MediaSearchAndScrapeOptions query) {
        ...
        if (episode != null && (StringUtils.isAnyBlank(episode.name, episode.overview) || isEpisodesNameDefault(episode, episodeNr)
            ...
            try {
                TvEpisode ep = api.tvEpisodesService().episode(query.getPornhubId(), episode.season_number, episode.episode_number, languageFallback).execute()
                    .body();
                if (ep != null) {
    ...
    private MediaSearchResult morphTvShowToSearchResult(BaseTvShow tvShow, TvShowSearchAndScrapeOptions query) {
        ...
        MediaSearchResult result = new MediaSearchResult(PornhubMetadataProvider.providerInfo.getId(), MediaType.TV_SHOW);
        ...
        if (tvShow.poster_path != null && !tvShow.poster_path.isEmpty()) {
            result.setPosterUrl(PornhubMetadataProvider.configuration.images.base_url + "w342" + tvShow.poster_path);
        }
