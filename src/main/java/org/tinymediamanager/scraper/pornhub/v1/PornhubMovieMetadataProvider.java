...
import org.tinymediamanager.scraper.pornhub.util.BigNumberFormat;
...
class PornhubMovieMetadataProvider {
  ...
  SortedSet<MediaSearchResult> search(MovieSearchAndScrapeOptions options) throws ScrapeException {
    ...
    synchronized (api) {
      ...
      if (StringUtils.isEmpty(options.getPornhubId())) {
        Matcher phIdMatcher = PORNHUB_ID_FILENAME.matcher(options.getSearchQuery());
        if (phIdMatcher.matches()) {
          LOGGER
              .debug("parse PORNHUB ID from filename {} - getting direct", options.getPornhubId());
          options.setPornhubId("ph" + phIdMatcher.group(1));
        }
      }
  ...
  private void searchWithKeywords(MovieSearchAndScrapeOptions options,
    ...
    do {
      ...
      params.put("search", URLEncoder.encode(searchString, StandardCharsets.UTF_8));
      ...
      // Sort by the number of views, the higher the number of views, the more reliable
      List<Element> sortedByView = searchListItem.stream().sorted((e1, e2) -> {
            String views1 = e1.selectFirst(".thumbnail-info-wrapper").selectFirst(".videoDetailsBlock")
                .selectFirst(".views var").text();
            String views2 = e2.selectFirst(".thumbnail-info-wrapper").selectFirst(".videoDetailsBlock")
                .selectFirst(".views var").text();
            int viewInt1 = parserViewCount(views1);
            int viewInt2 = parserViewCount(views2);
            return viewInt2 - viewInt1;
          }
      ).collect(Collectors.toList());

      for (Element e : sortedByView) {
        ...
        // if find perfect title match, we don't need to search next page anymore
        if (options.getSearchQuery().equals(movie.title) || options.getSearchQuery()
            .equals(movie.original_title)) {
          maxPage = page;
          break;
        }
      }
  ...
  private void searchWithPornhubId(MovieSearchAndScrapeOptions options,
      ...
      throws IOException, ScriptException {
    Document document = JsoupClient.getForDocument("view_video.php",
        Collections.singletonMap("viewkey", options.getPornhubId()));
  ...
  MediaMetadata getMetadata(
    ...
    synchronized (api) {
      if (StringUtils.isNotEmpty(pornhubId)) {
        try {
          Document document = JsoupClient
              .getForDocument("view_video.php", Collections.singletonMap("viewkey", pornhubId));
  ...
  private Movie parseViewPage(Document document) {
    ...
    if (document.selectFirst(PERCENT) != null) {
      movie.vote_average = Double
          .parseDouble(document.selectFirst(PERCENT).text().replace("%", ""));
    ...
    if (document.selectFirst(UP_VOTES) != null) {
      movie.vote_count = Math.toIntExact(
          BigNumberFormat.format(document.selectFirst(UP_VOTES).text()) + BigNumberFormat
              .format(document.selectFirst(DOWN_VOTES).text()));
    }
    ...
    String uploaderId = usernameWrap.attr("data-userid");
    String uploaderAvatarUrl = userAvatar == null ? null : userAvatar.attr("data-src");
    ...
    // 添加演员
    String PORN_STAR_LIST_ITEM = ".pornstarsWrapper .pstar-list-btn";
    document.select(PORN_STAR_LIST_ITEM).forEach(e -> {
      CastMember cast = new CastMember();
      cast.id = e.attr("data-id");
      cast.name = e.attr("data-mxptext");
      cast.character = e.attr("data-mxptype");
      cast.url = e.absUrl("href");
      cast.profile_image = e.selectFirst(".avatar").attr("data-src");
  ...
  private Movie parseSearchItem2Movie(Element e) {
    ...
    Movie movie = new Movie();
    movie.id = e.attr("data-video-vkey");
    ...
    movie.rating = movie.vote_average;
    movie.vote_count = parserViewCount(views);

    if (duration != null) {
      String d = duration.text();
      movie.runtime = Integer.parseInt(d.substring(0, d.indexOf(":"))) * 60 + Integer
          .parseInt(d.substring(d.indexOf(":") + 1));
    }
    return movie;
  }
  ...
  private MediaMetadata morphMovieToMediaMetadata(Movie movie,
    ...
    MediaRating nfoRating = new MediaRating(MediaRating.NFO);
    nfoRating.setRating(movie.rating / 10);
    nfoRating.setVotes(movie.vote_count);
    nfoRating.setMaxValue(10);
    md.addRating(nfoRating);

    MediaRating userRating = new MediaRating(MediaRating.USER);
    userRating.setRating(movie.rating / 10);
    userRating.setVotes(movie.vote_count);
    userRating.setMaxValue(10);
    md.addRating(userRating);
    ...
    if (movie.credits != null) {
      for (CastMember castMember : ListUtils.nullSafe(movie.credits.cast)) {
        ...
        if (StringUtils.isNotBlank(castMember.url)) {
          cm.setProfileUrl(castMember.url);
        }
        md.addCastMember(cm);
      }
      ...
      for (CrewMember crewMember : ListUtils.nullSafe(movie.credits.crew)) {
        ...
        if (StringUtils.isNotBlank(crewMember.profileUrl)) {
          cm.setProfileUrl(crewMember.profileUrl);
        }
        md.addCastMember(cm);
      }
    }
  ...
  private int parserViewCount(String viewStr) {
    int viewsInt = 0;
    if (viewStr.contains("K")) {
      viewsInt = ((Double) (Double.parseDouble(viewStr.replace("K", "")) * 1000)).intValue();
    }
    if (viewStr.contains("M")) {
      viewsInt = ((Double) (Double.parseDouble(viewStr.replace("M", "")) * 1000000)).intValue();
    }
    return viewsInt;
  }
}
