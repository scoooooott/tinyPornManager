...
class PornhubMovieSetMetadataProvider {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(PornhubMovieSetMetadataProvider.class);
   ...
   * @param original       the original movie set list
   * @param movieSetsFound the list that movie sets will be added.
   */
  private boolean verifyMovieSetTitleLanguage(List<MediaSearchResult> movieSetsFound,
      CollectionResultsPage original,
      MovieSetSearchAndScrapeOptions query) throws Exception {

    String lang = MediaLanguages.get(providerInfo.getConfig().getValue("titleFallbackLanguage"))
        .name().replace("_", "-");
    CollectionResultsPage fallBackResultsPage = api.searchService()
        .collection(query.getSearchQuery(), 1, lang).execute().body();

    if (fallBackResultsPage != null && original.results != null
        && fallBackResultsPage.results != null) {
      ...
      for (int i = 0; i < original.results.size(); i++) {
        ...
        if (MetadataUtil.calculateScore(query.getSearchQuery(), originalCollection.name)
            >= MetadataUtil.calculateScore(query.getSearchQuery(),
            fallbackCollection.name)) {
          searchResult.setTitle(originalCollection.name);
          searchResult.setPosterUrl(PornhubMetadataProvider.configuration.images.base_url + "w342"
              + originalCollection.poster_path);
          searchResult.setScore(
              MetadataUtil.calculateScore(query.getSearchQuery(), originalCollection.name));
        } else {
          searchResult.setTitle(fallbackCollection.name);
          searchResult.setPosterUrl(PornhubMetadataProvider.configuration.images.base_url + "w342"
              + fallbackCollection.poster_path);
          searchResult.setScore(
              MetadataUtil.calculateScore(query.getSearchQuery(), fallbackCollection.name));
        }
        movieSetsFound.add(searchResult);
      }
      return true;
   ...
   */
  MediaMetadata getMetadata(MovieSetSearchAndScrapeOptions options)
      throws MissingIdException, ScrapeException, NothingFoundException {
    ...
    synchronized (api) {
      try {
        ...
        if (collection != null && (StringUtils.isBlank(collection.overview) || StringUtils
            .isBlank(collection.name))
            && providerInfo.getConfig().getValueAsBool("titleFallback")) {

          String fallbackLang = MediaLanguages
              .get(providerInfo.getConfig().getValue("titleFallbackLanguage")).name()
              .replace("_", "-");
          Collection collectionInFallbackLanguage = api.collectionService()
              .summary(pornhubId, fallbackLang).execute().body();

          if (collectionInFallbackLanguage != null) {
            Collection collectionInDefaultLanguage = null;
            if (StringUtils.isBlank(collectionInFallbackLanguage.name) || StringUtils
                .isBlank(collectionInFallbackLanguage.overview)) {
              collectionInDefaultLanguage = api.collectionService().summary(pornhubId, null)
                  .execute().body();

            }

            if (StringUtils.isBlank(collection.name) && StringUtils
                .isNotBlank(collectionInFallbackLanguage.name)) {
            ...
            if (StringUtils.isBlank(collection.overview) && StringUtils
                .isNotBlank(collectionInFallbackLanguage.overview)) {
              collection.overview = collectionInFallbackLanguage.overview;
            } else if (StringUtils.isBlank(collection.overview)
                && collectionInDefaultLanguage != null
            ...
            for (BaseMovie movie : collection.parts) {
              for (BaseMovie fallbackMovie : collectionInFallbackLanguage.parts) {
                if (movie.id.equals(fallbackMovie.id)) {
                  if (StringUtils.isBlank(movie.overview) && !StringUtils
                      .isBlank(fallbackMovie.overview)) {
                    movie.overview = fallbackMovie.overview;
                  }
                  if (movie.title.equals(movie.original_title) && !movie.original_language
                      .equals(options.getLanguage().getLanguage())
    ...
    if (StringUtils.isNotBlank(collection.poster_path)) {
      ...
      ma.setPreviewUrl(
          PornhubMetadataProvider.configuration.images.base_url + "w185" + collection.poster_path);
      ma.setDefaultUrl(PornhubMetadataProvider.configuration.images.base_url + "original"
          + collection.poster_path);
    ...
    if (StringUtils.isNotBlank(collection.backdrop_path)) {
      MediaArtwork ma = new MediaArtwork(PornhubMetadataProvider.providerInfo.getId(),
          MediaArtwork.MediaArtworkType.BACKGROUND);
      ma.setPreviewUrl(PornhubMetadataProvider.configuration.images.base_url + "w300"
    ...
    for (BaseMovie part : ListUtils.nullSafe(collection.parts)) {
      ...
      if (StringUtils.isNotBlank(part.poster_path)) {
        MediaArtwork ma = new MediaArtwork(PornhubMetadataProvider.providerInfo.getId(),
            MediaArtwork.MediaArtworkType.POSTER);
        ma.setPreviewUrl(
            PornhubMetadataProvider.configuration.images.base_url + "w185" + part.poster_path);
        ma.setDefaultUrl(
            PornhubMetadataProvider.configuration.images.base_url + "original" + part.poster_path);
        ma.setLanguage(options.getLanguage().getLanguage());
      ...
      if (StringUtils.isNotBlank(part.backdrop_path)) {
        MediaArtwork ma = new MediaArtwork(PornhubMetadataProvider.providerInfo.getId(),
            MediaArtwork.MediaArtworkType.BACKGROUND);
        ma.setPreviewUrl(
            PornhubMetadataProvider.configuration.images.base_url + "w300" + part.backdrop_path);
        ma.setDefaultUrl(PornhubMetadataProvider.configuration.images.base_url + "original"
            + part.backdrop_path);
        ma.setLanguage(options.getLanguage().getLanguage());
        ma.setPornhubId(part.id);
