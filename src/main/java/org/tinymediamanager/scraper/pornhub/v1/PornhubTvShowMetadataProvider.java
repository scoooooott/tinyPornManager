...
import com.scott.pornhub.entities.BaseTvEpisode;
import com.scott.pornhub.entities.BaseTvShow;
import com.scott.pornhub.entities.CastMember;
import com.scott.pornhub.entities.ContentRating;
...
import com.scott.pornhub.entities.TvShow;
...
class PornhubTvShowMetadataProvider {

    private static final Logger LOGGER = LoggerFactory.getLogger(PornhubTvShowMetadataProvider.class);

    private final Pornhub api;

    PornhubTvShowMetadataProvider(Pornhub api) {
        this.api = api;
    }
    ...
    SortedSet<MediaSearchResult> search(TvShowSearchAndScrapeOptions options) throws ScrapeException {
        ...
        synchronized (api) {
            ...
            if (StringUtils.isNotEmpty(pornhubId)) {
                ...
                try {
                    Response<TvShow> httpResponse = api.tvService()
                        .tv(pornhubId, language, new AppendToResponse(AppendToResponseItem.TRANSLATIONS))
                        .execute();
            ...
            if (results.isEmpty() && StringUtils.isNotEmpty(imdbId)) {
                ...
                try {
                    Response<FindResults> httpResponse = api.findService()
                        .find(imdbId, ExternalSource.IMDB_ID, language).execute();
                    if (!httpResponse.isSuccessful()) {
                        throw new HttpException(httpResponse.code(), httpResponse.message());
                    }
            ...
            if (results.isEmpty()) {
                try {
                    ...
                    do {
                        Response<TvShowResultsPage> httpResponse = api.searchService()
                            .tv(searchString, page, language, null).execute();
     ...
     *
     * @param options the scrape options for getting the episode list
     * @return the episode list
     * @throws ScrapeException    any exception which can be thrown while searching
     * @throws MissingIdException indicates that there was no usable id to scrape
     */
    List<MediaMetadata> getEpisodeList(MediaSearchAndScrapeOptions options)
        ...
        synchronized (api) {
            try {
                Response<TvShow> showResponse = api.tvService().tv(pornhubId, language).execute();
                ...
                for (TvSeason season : ListUtils.nullSafe(showResponse.body().seasons)) {
                    ...
                    Response<TvSeason> seasonResponse = api.tvSeasonsService()
                        .season(pornhubId, season.season_number, language,
                            new AppendToResponse(AppendToResponseItem.TRANSLATIONS)).execute();
    ...
    MediaMetadata getTvShowMetadata(
        TvShowSearchAndScrapeOptions options)
        ...
        synchronized (api) {
            try {
                Response<TvShow> httpResponse = api.tvService()
                    .tv(pornhubId, language, new AppendToResponse(AppendToResponseItem.TRANSLATIONS,
                        AppendToResponseItem.CREDITS, AppendToResponseItem.EXTERNAL_IDS,
                        AppendToResponseItem.CONTENT_RATINGS)).execute();
                if (!httpResponse.isSuccessful()) {
                    throw new HttpException(httpResponse.code(), httpResponse.message());
                }
        ...
        md.setId(PornhubMetadataProvider.providerInfo.getId(), pornhubId);
        ...
        for (String country : ListUtils.nullSafe(complete.origin_country)) {
            if (providerInfo.getConfig().getValueAsBool("scrapeLanguageNames")) {
                md.addCountry(LanguageUtils
                    .getLocalizedCountryForLanguage(options.getLanguage().toLocale(), country));
            }
            else {
        ...
        if (StringUtils.isNotBlank(complete.poster_path)) {
            ...
            ma.setPreviewUrl(
                PornhubMetadataProvider.configuration.images.base_url + "w185" + complete.poster_path);
            ma.setDefaultUrl(
                PornhubMetadataProvider.configuration.images.base_url + "w342" + complete.poster_path);
            ma.setLanguage(options.getLanguage().getLanguage());
            ma.setPornhubId(complete.id);
            md.addMediaArt(ma);
        }
        ...
        if (complete.content_ratings != null) {
            ...
            for (ContentRating country : ListUtils.nullSafe(complete.content_ratings.results)) {
                if (countryCode == null
                    || countryCode.getAlpha2().compareToIgnoreCase(country.iso_3166_1) == 0) {
                    // do not use any empty certifications
                    if (StringUtils.isEmpty(country.rating)) {
                        continue;
                    }
                    md.addCertification(
                        MediaCertification.getCertification(country.iso_3166_1, country.rating));
                }
            }
        }
    ...
    MediaMetadata getEpisodeMetadata(
        ...
        synchronized (api) {
            ...
            try {
                Response<TvSeason> httpResponse = api.tvSeasonsService()
                    .season(pornhubId, seasonNr, language,
                        new AppendToResponse(AppendToResponseItem.CREDITS)).execute();
                if (!httpResponse.isSuccessful()) {
                    throw new HttpException(httpResponse.code(), httpResponse.message());
                }
        ...
        md.setId(PornhubMetadataProvider.providerInfo.getId(), episode.id);
        ...
        if (StringUtils.isNotBlank(episode.still_path)) {
            MediaArtwork ma = new MediaArtwork(PornhubMetadataProvider.providerInfo.getId(),
                MediaArtworkType.THUMB);
            ma.setPreviewUrl(
                PornhubMetadataProvider.configuration.images.base_url + "original" + episode.still_path);
            ma.setDefaultUrl(
                PornhubMetadataProvider.configuration.images.base_url + "original" + episode.still_path);
            md.addMediaArt(ma);
        }
     ...
     */
    @Deprecated
    private String getPornhubIdFromImdbId(String imdbId) {
        try {
            ...
            if (findResults != null) {
                ...
                else if (findResults.tv_episode_results != null && !findResults.tv_episode_results
                    .isEmpty()) {
    ...
    /**
     * Language Fallback Mechanism - PORNHUB id lookup
     *
     * @param language the language to search for
     * @param show     the tv show to modify
     */
    private void verifyTvShowLanguageTitle(Locale language, TvShow show) {
        ...
        if (providerInfo.getConfig().getValueAsBool("titleFallback") || StringUtils
            .isEmpty(show.overview)) {
            Locale fallbackLanguage = Locale
                .forLanguageTag(providerInfo.getConfig().getValue("titleFallbackLanguage"));

            if ((show.name.equals(show.original_name) && !show.original_language
                .equals(language.getLanguage())) && !language.equals(fallbackLanguage)) {
                ...
                String[] temp = PornhubMetadataProvider
                    .getValuesFromTranslation(show.translations, fallbackLanguage);
     ...
     * @param language the language to search for
     * @param show     the show
     * @throws IOException any IOException occurred
     */
    private void verifyTvShowLanguageTitle(Locale language, BaseTvShow show) throws IOException {
        ...
        if (providerInfo.getConfig().getValueAsBool("titleFallback")) {
            ...
            if ((show.name.equals(show.original_name) && !show.original_language
                ...
                Response<TvShow> httpResponse = api.tvService()
                    .tv(show.id, lang, new AppendToResponse(AppendToResponseItem.TRANSLATIONS)).execute();
                if (!httpResponse.isSuccessful()) {
                    throw new HttpException(httpResponse.code(), httpResponse.message());
                }
                ...
                // merge empty ones with fallback
                String[] temp = PornhubMetadataProvider
                    .getValuesFromTranslation(s.translations, fallbackLanguage);
                if (StringUtils.isBlank(val[0])) {
                    val[0] = temp[0];
                }
                if (StringUtils.isBlank(val[1])) {
     ...
     * @param episode the original tv episode
     */
    private void verifyTvEpisodeTitleLanguage(BaseTvEpisode episode,
        MediaSearchAndScrapeOptions query) {
        ...
        if (episode != null && (StringUtils.isAnyBlank(episode.name, episode.overview)
            || isEpisodesNameDefault(episode, episodeNr)
            || providerInfo.getConfig().getValueAsBool("titleFallback"))) {

            String languageFallback = MediaLanguages
                .get(providerInfo.getConfig().getValue("titleFallbackLanguage")).name().replace("_", "-");

            try {
                TvEpisode ep = api.tvEpisodesService()
                    .episode(query.getPornhubId(), episode.season_number, episode.episode_number,
                        languageFallback).execute()
                    .body();
                if (ep != null) {
                    if ((ep.season_number == seasonNr || ep.episode_number.equals(episode.season_number))
                        && (ep.episode_number == episodeNr || ep.episode_number
                        .equals(episode.episode_number))) {

                        if (StringUtils.isBlank(episode.name) || (isEpisodesNameDefault(episode, episodeNr)
                            && !isEpisodesNameDefault(ep, episodeNr))) {
    ...
    private Boolean isEpisodesNameDefault(BaseTvEpisode episode, Integer episodeNr) {
        ...
        return (originalEpisodeName = episode.name.split(" ")).length == 2
            && (potentialEpisodeNumber = toInteger(originalEpisodeName[1])) != null
            && (potentialEpisodeNumber.equals(episode.episode_number) || potentialEpisodeNumber
            .equals(episodeNr));
    }

    private MediaMetadata morphTvEpisodeToMediaMetadata(BaseTvEpisode episode) {
        MediaMetadata ep = new MediaMetadata(PornhubMetadataProvider.providerInfo.getId());
    ...
    private MediaSearchResult morphTvShowToSearchResult(BaseTvShow tvShow,
        TvShowSearchAndScrapeOptions query) {

        MediaSearchResult result = new MediaSearchResult(PornhubMetadataProvider.providerInfo.getId(),
            MediaType.TV_SHOW);
        result.setId(tvShow.id);
        ...
        if (tvShow.poster_path != null && !tvShow.poster_path.isEmpty()) {
            result.setPosterUrl(
                PornhubMetadataProvider.configuration.images.base_url + "w342" + tvShow.poster_path);
        }
        ...
        if ((StringUtils.isNotBlank(query.getImdbId()) && query.getImdbId().equals(result.getIMDBId()))
            || String.valueOf(query.getPornhubId()).equals(result.getId())) {
