 ...
 * <p>
 * <code>BaseRootPaneUI</code> provides support for the <code>windowDecorationStyle</code> property of <code>JRootPane</code>.
 * <code>BaseRootPaneUI</code> does this by way of installing a custom <code>LayoutManager</code>, a private <code>Component</code> to render the
 * appropriate widgets, and a private <code>Border</code>. The <code>LayoutManager</code> is always installed, regardless of the value of the
 * <code>windowDecorationStyle</code> property, but the <code>Border</code> and <code>Component</code> are only installed/added if the
...
public class BaseRootPaneUI extends BasicRootPaneUI {
  ...
  /**
   * Sets the window title pane -- the JComponent used to provide a plaf a way to override the native operating system's window title pane with one
  ...
  /**
   * Returns the <code>BaseTitlePane</code> rendering the title pane. If this returns null, it implies there is no need to render window decorations.
   *
  ...
  private class MouseInputHandler implements MouseInputListener {
    ...
    /**
     * Returns an integer indicating the position of <code>spot</code> in <code>width</code>. The return value will be: 0 if < BORDER_DRAG_THICKNESS 1
     * if < CORNER_DRAG_WIDTH 2 if >= CORNER_DRAG_WIDTH && < width - BORDER_DRAG_THICKNESS 3 if >= width - CORNER_DRAG_WIDTH 4 if >= width -
